...
function Failure (message, sff) {
  if (!(this instanceof Failure)) {
    return new Failure(message, ssf || Failure);
  }
...
// Trim frames beyond the provided stack first function
...
function makeFramesGetter (ssf) {
  // V8 code path using its callsite API
  if (typeof Failure.Error.captureStackTrace === 'function') {
    Failure.Error.captureStackTrace(V8FRAMES, ssf || makeFramesGetter);
  ...
  return function () {
    ...
    return trimFrames(frames.slice(2), ssf);
  };
}
...
proto.prepareStackTrace = function (frames) {
  ...
  frames.forEach(function (frame) {
    ...
    if (fn.__frames__) {
      var nested = fn.__frames__();
      var s = 'NESTED!';
      nested.forEach( function (frame) {
        s += '\n at ' + frame;
      });
      console.log(s);
    }
  ...
  samefile = frames.filter(function (frame) {
    return frame.getFileName() === target.getFileName();
  });
