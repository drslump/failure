...
function Failure (message, sff) {
  ...
  if (typeof Object.defineProperty !== 'function') {
    ...
    this.stack = this.prepareStackTrace(this.frames);
  }
...
function unwind (frames) {
  ...
  for (var i=0, fn; i < frames.length; i++) {
    ...
    if (fn && fn['failure:frames']) {
      ...
      // Call the getter and keep a reference to the result
...
// Wraps the function beforing annotating it with tracking information, this
// allows to track multiple schedullings of a single functions.
...
Failure.patch = function Failure_patch(obj, name, idx) {
  ...
  obj[name] = function () {
    arguments[idx] = Failure.track(arguments[idx], Failure_patch);
